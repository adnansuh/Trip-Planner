...
const getMockItinerary = (
  ...
  const activityPools: Record<string, Array<{ title: string; desc: string; cost: number }>> = {
    food: [
  ...
  const getInterestPool = (dayIdx: number): Array<{ title: string; desc: string; cost: number }> => {
  ...
  if (interests.includes('Adventure') || interests.includes('Adventure & Outdoor')) {
    basePacking.push(
      { id: 'pack-5', name: 'Hiking boots / Trail shoes', category: 'Clothing', packed: false },
      { id: 'pack-6', name: 'Refillable water bottle', category: 'Essentials', packed: false },
      { id: 'pack-7', name: 'Water-resistant backpack', category: 'Activities', packed: false }
    );
  }

  if (interests.includes('Food') || interests.includes('Culture')) {
    basePacking.push(
      { id: 'pack-8', name: 'Hand sanitizer & wet wipes', category: 'Toiletries', packed: false },
      { id: 'pack-9', name: 'Camera or phone lens kit', category: 'Electronics', packed: false }
    );
  }

  if (interests.includes('Shopping')) {
    basePacking.push(
      { id: 'pack-10', name: 'Reusable canvas shopping bags', category: 'Activities', packed: false }
    );
  }
...
export const generateTripItinerary = async (
  ...
  try {
    ...
    if (data.packingList && Array.isArray(data.packingList)) {
      data.packingList = data.packingList.map((item: any, idx: number) => ({
